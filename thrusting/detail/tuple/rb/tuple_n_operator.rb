...
__host__ __device__
typename tuple#{n}<T>::type operator#{op}(const typename tuple#{n}<T>::type &x, const U &y){
...
template<typename T, typename U>
__host__ __device__
typename tuple#{n}<T>::type operator*(const U &n, const typename tuple#{n}<T>::type &x){
  return x * n;
}
#{_operator(n, "/")}
"""
